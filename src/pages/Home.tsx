...
const SHOWCASE_CONFIGS: (Partial<QROptions> & { label: string })[] = [
  ...
  },
  {
    ...
    foregroundColor: {
      type: 'gradient',
      gradient: { type: 'linear', stops: [{ offset: 0, color: '#059669' }, { offset: 1, color: '#0891b2' }], rotation: 90 },
    },
    backgroundColor: { type: 'solid', color: '#ffffff' },
  },
  {
...
export default function Home() {
  ...
  const heroConfigs = [SHOWCASE_CONFIGS[1], SHOWCASE_CONFIGS[2], SHOWCASE_CONFIGS[3]]
            ...
            <table className="w-full text-sm" aria-label="Pricing comparison">
              <thead>
                <tr className="bg-slate-50 text-left">
                  <th className="px-6 py-4 font-semibold text-slate-900">Service</th>
                  <th className="px-6 py-4 font-semibold text-slate-900">Price</th>
                  <th className="px-6 py-4 font-semibold text-slate-900 hidden sm:table-cell">Model</th>
                </tr>
              </thead>
              <tbody>
                <tr className="bg-brand-50 border-t border-brand-100">
                  <td className="px-6 py-4 font-semibold text-brand-700">QR Code Generator (us)</td>
                ...
                {COMPETITORS.map((c) => (
                  <tr key={c.name} className="border-t border-slate-100">
                    ...
                    <td className="px-6 py-4 text-slate-500">
                      {c.price}
                      <span className="block text-xs text-slate-400 sm:hidden">{c.note}</span>
                    </td>
                    <td className="px-6 py-4 text-slate-400 hidden sm:table-cell">{c.note}</td>
                  </tr>
                ))}
              </tbody>
            </table>
